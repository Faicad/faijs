...
- **第七批补投 `io` 模块**（28 个符号：12 个 type re-export + 16 个 skip）。`export*`/`import*` 家族**不直接投影**——文件/字节交换属于 faijs `node-host` / `browser-host` 端口适配（P14 采纳的 IO 裁决），而非静态透传；其中若干还消费非 faijs 输入（`exportOBJ` 收 `ShapeMesh`、`exportDXF` 收 `DXFEntity[]`），`importSVG`/`importSVGPathD` 返回 kernel-句柄 `Blueprint`（2d 家族）。16 个值全部 `skip` + reason，指向 host ports 路线。12 个类型（`DXFEntity`、`DXFExportOptions`、`DXFImportOptions`、`GltfExportOptions`、`GltfFace`、`GltfMaterial`、`MaterialFn`、`StepExportOptions`、`StepExportPart`、`SVGImportOptions`、`ThreeMFExportOptions`、`ThreeMFMaterial`）照常 re-export，faijs 今天就能对宿主端口数据做类型标注。进度：io 12 + 16 skip = 28/28 已入册。
