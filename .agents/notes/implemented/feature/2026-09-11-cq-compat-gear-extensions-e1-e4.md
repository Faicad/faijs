...
Add the four primitives inside cq-compat as thin wrappers, so `fai_cq_gears` consumes them through the cq-compat API instead of touching `occt-wasm` directly. Each op lives in `packages/cq-compat/src/workplane.ts` as an `export async function`, calls the native kernel via the shared `getKernel()` singleton (`getKernel() as unknown as OcctKernel`), and is exported from `index.ts`. `helix`→`makeHelixWire`, `splitFace`→`halfSpace`+`split`+`getSubShapes`, `twistExtrude`→ native `rotate`/`translate` sweep through `loft`. `splineFace` took two passes: the first cut called `bsplineSurface` over the whole grid, but occt-wasm exposes no DegMin/DegMax/Tol3D there, so it ran with kernel defaults and measured 2.269e-4 relative area against CadQuery `makeSplineApprox` — 7 orders off the 4.2e-11 the plan requires. It now defaults to `row-approx-loft` (per-row `approximatePoints(tol=1e-2)` + `loft`), measured 4.2e-11 straight / 5.6e-7 helical, with the one-shot grid fit kept as an opt-in `strategy: 'grid'`. All four carry full JSDoc (repo-wide `verify-export-jsdoc` gate). Unit tests (4 files, 11 cases) build base geometry through the native kernel and assert extents/area; they pass, and the full cq-compat suite (122 tests) is green with no regression.
