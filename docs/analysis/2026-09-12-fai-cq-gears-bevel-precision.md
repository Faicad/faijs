# fai_cq_gears BevelGear 移植实测：几何偏差来源与量级

日期：2026-09-12
状态：**实测完成；T1 bbox 绝对门禁与 T2 体积容差两项待拍板**
关联方案：`docs/plans/2026-09-11-fai-cq-gears-port.md`（§8.2 BevelGear、§9.1 T1、§9.2 T2、§10 降级阶梯）
代码位置：`packages/fai_cq_gears/src/bevel_gear.ts`、`src/profile.ts::bevelGearGeometry`
...
| ② | T2 体积容差：case08 实测 5.9e-5 | 相对 5.9e-5（斜齿 1.9e-4） | `CALIBRATED_COMPARE.volumeRelativeTolerance` = **1e-6** |
