...
- **Global solver** (`solver: 'global'`): an alternative pure-TS kernel (`api/assembly/solvers/global-solver.ts`, Levenberg-Marquardt over a central-difference Jacobian) that replicates the CadQuery `solver.py` cost semantics (9 cost forms, quaternion magnitude parameterization, `Plane = Axis(π)+Point(0)`, `Axis = Axis(0)+Point(0)`, bounding-box-diagonal scale). Selected via `cad.assembly({ solver: 'global' })`; it is the **default** when called through cq-compat's `buildAssembly`. Failure mode differs from the brepjs path: it never diverges, so `converged` is **always `true`** (LM termination is the stopping criterion). Per-constraint squared residuals are returned in `residuals` (one entry per constraint, **omitted on the brepjs path** — its presence is the wire signal that the global path ran). Unsupported entity types surface as `warnings` (and in `unsupported`) rather than throwing, except genuinely unresolvable entity references which throw `E_TOPO_NOT_FOUND`. Anchoring follows CadQuery B4: `fixed` member(s) and the assembly's own name are frozen as identity; otherwise the first binary member is the anchor. Rotation pivot is always `[0,0,0]` (CadQuery `Rotate(v,R)+T` about the world origin), so a free member's transform is `p' = R·p + translation`.
