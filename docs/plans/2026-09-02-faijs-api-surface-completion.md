...
| **P14** | **新增符号全量投影**：按模块分批（`operations` 122 → `core` 129 → `sketching` 51 → `2d` 37 → `io`/`measurement`/`gear`/`query`/`projection`/`text`）；每批跑 upstream 测试。**不含 §2.6 排除的四个模块**。**✅ 已投六片**：`measurement`（20 投影 + 1 skip，全 query/type）、`text`（5 + 3 skip）、`projection`（7 + 2 skip）、`query`（8 + 6 skip，finder 状态化工具）、`ns`（9 投影，命名空间整包 re-export）、`gear`（14 投影 + 3 skip，复合多句柄结果判 skip）——生成器多模块化 + query 位置泛化 + pure/type/namespace/复合结果分类路径验证 | 中 | P13 通过 |
