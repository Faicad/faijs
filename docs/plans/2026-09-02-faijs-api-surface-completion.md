...
- **方向层被 `docs/plans/2026-09-03-faijs-brepjs-compat-api.md`（v3）取代**：本方案的「throw 主面 +
  ...
  （vendored 树、生成器与 arg-spec 适配表、符号清单、生成面 441 符号、fai_* 更名、守卫脚本、
  TPMS 模板）；未接线部分（生成面未接入导出面/cad 面）由 v3 P21–P24 按新方向接线。
...
| **P14** | **新增符号全量投影**：按模块分批（`operations` 122 → `core` 129 → `sketching` 51 → `2d` 37 → `io`/`measurement`/`gear`/`query`/`projection`/`text`）；每批跑 upstream 测试。**不含 §2.6 排除的四个模块**。**✅ 已投八片**：`measurement`（20 + 1 skip，全 query/type）、`text`（5 + 3 skip）、`projection`（7 + 2 skip）、`query`（8 + 6 skip，finder DSL）、`ns`（9 投影，命名空间整包 re-export）、`gear`（14 + 3 skip，复合多句柄结果判 skip）、`2d`（12 type + 33 skip，kernel-2D 制图框架整体判 skip）、`io`（12 type + 15 skip，export*/import* 走 host ports，§5.5）——生成器多模块化 + query 位置泛化 + pure/type/namespace/复合结果/宿主 DSL/host-IO 分类路径验证 | 中 | P13 通过 |
