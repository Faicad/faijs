...
状态：实施中（P1–P4 guarded + P5 引擎侧已落地：MetadataExtractor + DirectExecutor + computeLiveShapes，对拍 A-16/A-17/A-14 全绿；CadRuntime 增 `CadRuntimeOptions.executor:'direct'` 可选无 IR 执行通道——execute/append/update 分支 + computeLiveShapes 终端组装 + failedAt.lineNo + AppendPrefixError 语义保留 + E4 执行选项（beforeStatement/executionTimeoutMs→E_EXEC_LIMIT），runtime 面对拍 49 例绿 + direct 面 E6/E7 已接（failedAt.index 场景语句序数 / check() 语法门禁降级）；多文件引擎侧（§4.5/A-8/A-9/A-10）：`HostPorts.projectLoader` + `cad-runtime/module-registry.ts`（DFS 装载、循环检测、绑定 gate：绑定 ∈ liveShapes∪fns、独立 ctx 逐模块执行、liveShapes 导出）+ DirectExecutor 顶层 import 模块作用域解析（parseBody ESM 优先）+ import 绑定 ctx 预置（opts.imports）+ 成员访问 ctx 优先 + 提取器命名空间成员引用放行，`no-ir/multifile` 8 例 + A-5 参数保真 1 例绿（core module-registry.test 10 例）——宿主 projectLoader 注入（3d_editor 侧）与参数编辑 UI 面未接；**缺省仍为 'module' 未翻转**——direct 组装尚未复刻 BREP topology/naming/changed/activeValues 面，完整 P4 翻转（collectResult 缺省换源 + check 降级语法门禁 + 旧路径停用）与 P6 未实施）
