...
版本状态（2026-08-28 重写）：本文档随 **语言正常化重构**（消灭 op 概念）与 **DAG 终端判定恢复** 两轮改造重写。与旧版的关键差异：① 废弃 `partN_vM` 版本链命名，改为**可重赋值的 `partN`**（模型即变量）；② `op` 概念消灭，函数就是函数（`callee`），引擎零函数知识；③ 语句身份 `StmtId`（`sN`）与变量身份 `PartName` 分离；④ 终端判定恢复为 **DAG 活跃性**（最后写者 + 下游无消费），由符号表的 readonly 数据驱动。
