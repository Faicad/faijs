...
export function generateModel(
  ...
  if (bodiesWithGeo.size > 0) {
    ...
    // main.fai.js: aggregate the per-Body terminals via cad.group
    const lines: string[] = [];
    ...
    const members = [...bodiesWithGeo].map((b, i) => {
      // import-free contract: each Body file is a separate entry executed by
      // the loader; here we reference the chain-head names it produces.
      return `${b}_out`;
    });
    if (members.length > 1) {
      rootVar = 'part_out';
      ...
      rootVar = members[0];
      lines.push(`// single Body: terminal is ${rootVar} (model/${members[0]}.fai.js)`);
    }
    code = lines.join('\n') + '\n';
  } else {
