 ...
 *
 * 验收判据（§5.4）：
 * - `import * as mech from 'mech-lib'` + `mech.makeHeadstock(...)` → parse → codegen
 ...
 * - statementKey：`cad.chamfer` ≠ `mech-lib.chamfer`（包名前缀）。
 */
...
describe('F2: import + 命名空间往返', () => {
  it('import * as mech + mech.makeHeadstock → 往返保留 import 段与命名空间前缀', () => {
    ...
    expect(script.imports).toEqual([{ specifier: 'mech-lib', kind: 'namespace', localName: 'mech', packageName: 'mech-lib' }])
  ...
  it('named / default import 往返保真', () => {
    const code = [
      "import { makeHeadstock, gear } from 'gear-lib'",
  ...
  it('StatementSummary 带 namespace/packageName（timeline「带包名」标识来源）', () => {
    ...
    expect(summaries[1].packageName).toBe('mech-lib')
  })
})
...
describe('F2: import 约束', () => {
  ...
  it('动态 import() 仍报 E_CONTROL_FLOW（import 段之外）', () => {
    let err: ParseError | undefined
    try {
      parseScript('let part0 = await import("mech-lib")')
    } catch (e) {
      err = e as ParseError
    }
  ...
  it('export 仍被拒绝（E_STATEMENT）', () => {
    let err: ParseError | undefined
    try {
      parseScript("import * as mech from 'mech-lib'\nexport const x = 1")
    } catch (e) {
      err = e as ParseError
    }
...
describe('F2: mock 库端到端执行', () => {
  ...
  it('未登记 specifier 的命名空间调用 → check ② 明确报错（不回退、不静默）', () => {
    const runtime = createRuntime(createNodePorts())
    const res = runtime.check("import * as mech from 'mech-lib'\nlet part0 = mech.makeHeadstock()")
    expect(res.ok).toBe(false)
    const err = res.errors.find((e) => e.stage === 'symbol')
    expect(err).toBeDefined()
    expect(err!.message).toMatch(/namespace "mech" is not registered/)
    runtime.dispose()
  })

  it('已登记库但 callee 不存在 → check ② 报函数不存在', () => {
    const runtime = createRuntime(createNodePorts())
    runtime.registerLib('mech', { makeHeadstock: () => solid(cubeMesh(1)) })
    const res = runtime.check("import * as mech from 'mech-lib'\nlet part0 = mech.nope()")
    expect(res.ok).toBe(false)
    const err = res.errors.find((e) => e.stage === 'symbol')
