...
// FAIIS-CUT: kernel init machinery omitted per D10 — init/initFromOC/initFromManifold/
// prewarm/withKernel/withQuality/registerKernelTier/getKernelTier/withTier/BrepkitAdapter/
// OcctWasmAdapter (runtime-switching + 3-tier WASM fallback cut; faijs binds the single
// occt-wasm instance via initOcctWasm and freezes the registry).
export { getKernel, registerKernel, getKernelCapabilities } from './kernel/index.js';
...
export type {
  KernelAdapter,
  // FAIIS-CUT: BrepkitHandle/OcctWasmHandle/OcctWasmModule/OcctKernelWasm omitted per D10
  SurfaceType,
  ShapeOrientation,
  ShapeType,
...
// FAIIS-CUT: perfStats module omitted (not ported; kernel/index.ts has no perfStats exports)
...
// FAIIS-CUT: voxel module omitted per §2.6.2 (depends on unpublished brepjs-voxel-wasm)

// ── Layer 2: implicit (ADR-0013, field-first SDF) ──

// FAIIS-CUT: implicit module omitted per §2.6.3 (conflicts with faijs sdf)

// ── Layer 3: lattice (ADR-0013) ──

// FAIIS-CUT: lattice module omitted per §2.6.4 (conflicts with faijs sdf templates)
...
// FAIIS-CUT: blueprintContourFns omitted (U9-adjacent — depends on csg/segments.ts, see §2.6.1)
...
// FAIIS-CUT: worker module omitted per §2.6.5 (conflicts with faijs host-layer worker)
...
// FAIIS-CUT: csg namespace omitted per U9 (see §2.6.1)
