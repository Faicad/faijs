/**
 * folder-loader — 目录句柄 → DemoProjectLoader（自 core 迁入，行为逐字不变）。
 *
 ...
 */
import type { DemoProjectLoader, FsDirectoryHandleLike, FsFileHandleLike } from './types'
import { FAI_SUFFIX, DEFAULT_SKIP_DIRS, escapeError, errMessage, isSkippedDir } from './shared'

/** createFolderProjectLoader 构造选项。 */
...
/**
 * 构造基于目录句柄的 ProjectLoader（浏览器多文件，folder 通道）。
 *
...
export async function createFolderProjectLoader(
  ...
  /**
   * 按 key 逐段下钻到文件钩子：前 N-1 段 `getDirectoryHandle`，末段 `getFileHandle`。
