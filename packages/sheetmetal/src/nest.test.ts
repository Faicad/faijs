...
import { nest, nestToDXF, patternBbox } from './nestFns.js';
import type { AuthorSpec } from './authorFns.js';
import type { BendRule, FlatPattern } from './types.js';
...
describe('nest — bbox', () => {
  ...
  it('NO OVERLAP + WITHIN BOUNDS over all placement pairs (with margin + spacing)', () => {
    ...
    for (const s of r.value.sheets) {
      const rects = s.placements.map((p) => placedRect(parts[p.patternIndex] as FlatPattern, p.x, p.y, p.rotationDeg as 0 | 90));
