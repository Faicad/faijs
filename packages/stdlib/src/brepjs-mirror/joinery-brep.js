 ...
 * 3. 返回每分量的顶点索引集合
 */
function detectCapComponentsFromMesh(mesh, normal, originOffset) {
    const eps = 0.001;
 ...
 */
function computeCrossSectionCentroidFromMesh(mesh, normal, originOffset, fallback, comp) {
    const eps = 0.001;
 ...
 */
function computeCrossSectionWidthFromMesh(mesh, normal, originOffset, widthDir, comp) {
    const eps = 0.001;
    const { positions } = mesh;
...
function solidToMeshData(kernel, solid) {
 ...
 */
function makeBasisTransform(basis, origin) {
    const basisMatrix = new THREE.Matrix4().makeBasis(new THREE.Vector3(...basis.widthDir), new THREE.Vector3(...basis.depthDir), new THREE.Vector3(...basis.normal));
 ...
 */
export function buildWedgeSolid(kernel, basis, depth, width, angleDeg, extrudeLength) {
 ...
 */
export function buildDowelSolid(kernel, centroid, basis, diameter, height) {
 ...
 */
export function buildTenonSolid(kernel, centroid, basis, sideLength, height) {
    const half = sideLength / 2;
    // 在局部坐标系中创建盒子：X ∈ [-half, half], Y ∈ [-half, half], Z ∈ [-height, 0]
    // 局部: X → widthDir, Y → depthDir, Z → normal
    const localBox = kernel.makeBoxFromCorners({ x: -half, y: -half, z: -height }, { x: half, y: half, z: 0 });
    // 变换到世界坐标系
 ...
 */
export function detectCrossSectionComponents(kernel, upper, basis) {
 ...
 */
export function dovetailBooleanSplitBrep(kernel, original, basis, groove) {
    ...
    upper = newUpper;
    // Step 6: 构造公差楔
    const wedgeTol = buildWedgeSolid(kernel, basis, groove.depth + groove.depthTolerance, groove.width + groove.widthTolerance, groove.flapsAngle, extrudeLength + overhang);
 ...
 */
export function dowelOrTenonBooleanSplitBrep(kernel, original, basis, shape, params, selectedSections) {
...
//# sourceMappingURL=joinery-brep.js.map