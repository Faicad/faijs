...
function copyBrep(input) {
    const kernel = getBackends().kernel.occt;
    if (!kernel)
        throw new Error('[stdlib/copy] no OCCT kernel');
    const inputSolid = brepOf(input);
    if (!inputSolid)
        throw new Error('[stdlib/copy] input is not BREP');
...
export function copy(input) {
    if (!input)
        throw new Error('[stdlib/copy] no input geometry');
    keep(input);
