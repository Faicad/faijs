...
async function screwBrep(params) {
    const kernel = getBackends().kernel.occt;
    if (!kernel)
        throw new Error('[stdlib/screw] no OCCT kernel');
    const system = params.system;
    const specIdx = params.specIdx;
    const spec = getScrewSpec(system, specIdx);
    const length = params.length;
    const head = params.head;
    const thread = params.thread;
    const pitchCustom = params.pitchCustom;
    // 1. 螺杆（圆柱体，居中）
 ...
 * 构造六棱柱 BREP solid
 */
function makeHexPrismBrep(kernel, radius, height, zOffset) {
    // 六边形顶点
    const pts = [];
    for (let i = 0; i < 6; i++) {
...
export function assertScrewParams(params) {
    if (typeof params.system !== 'string' || typeof params.specIdx !== 'number') {
        throw new Error(`[stdlib/screw] system (string) and specIdx (number) are required, got system=${JSON.stringify(params.system)}, specIdx=${JSON.stringify(params.specIdx)}`);
    }
...
export async function screw(params) {
    ...
    return solid(await cad.screw({
        ...
        pitchCustom: params.pitchCustom,
