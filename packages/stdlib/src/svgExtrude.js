...
export function assertSvgExtrudeParams(params) {
...
function svgExtrudeBrep(params, svgText) {
    const kernel = getBackends().kernel.occt;
    if (!kernel)
        throw new Error('[stdlib/svgExtrude] no OCCT kernel');
...
export async function svgExtrude(params) {
    assertSvgExtrudeParams(params);
    // 解析 SVG 资产引用（AssetRef → SVG 文本）；两条路径都需要
    const svgText = await resolveSvgArg(params.svg, { assets: getBackends().assets });
...
//# sourceMappingURL=svgExtrude.js.map