...
// NOTE (P3 harness): our host kernel is D10-bound occt-wasm, so the divergence
// registry below is consulted with currentKernelId='occt-wasm' — exactly the
// branch brepjs's own CI uses for the occt-wasm adapter (e.g. fuse ignores the
// simplify option → booleanFns.pairwiseSimplifyFaceMerge skipped).
export const currentKernelId: string = 'occt-wasm';
