...
function renderDoc(locale: 'en' | 'zh'): string {
  ...
  lines.push('|---|---|---|---|')
  lines.push('| ① TS 兼容面 | 第三方库（TS 代码，如 gear-lib-demo / sheetmetal） | brepjs 原样：位置参数 + `Result` 原生；同名同签名；`Sketcher` / `Blueprint` / `draw` DSL；`ok` / `err` / `isErr` / `pipe` 组合子 | `@faicad/faijs` 主导出（`packages/core/src/api/compat/`） |')
  ...
  lines.push('| ③ 库边界面 | `registerLib` 注册的第三方库导出函数 | 库作者写纯 brepjs 代码；入口 `Shape` → 借入，出口 `Solid` → 收养，`Result` 原样传递 | `runtime.registerLib(binding, ns, { autoLift: true })` |')
  lines.push('')
