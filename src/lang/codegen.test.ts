/**
 * codegen 单元测试 — statementToLine / scriptToCode (Phase 3)
 *
 * Phase 3 命名规则：
 * - 单入单出 → 复用输入名，`let` 重赋值
...
describe('codegen: statementToLine 布尔 (Phase 3: 新名)', () => {
  it('boolean 归一取消：callee 直写 subtract', () => {
    ...
    expect(statementToLine(stmt)).toBe('let part2 = cad.subtract(part0, part1)')
  })
})

describe('codegen: statementToLine 雕刻 (Phase 3: 复用输入名)', () => {
  it('engrave：文本雕刻', () => {
    const stmt = makeStmt({
      id: 's2',
      callee: 'engrave',
    ...
    expect(statementToLine(stmt)).toBe("let part0 = cad.engrave(part0, { text:'Hello', depth:2, textSize:10 })")
  })
})
