...
describe('E15.1: 装配链式调用解析', () => {
  it('解析 let assem1 = cad.assemble({...})', () => {
    ...
    expect(assembleStmt.isMarker).toBe(true)
  ...
  it('解析 assem1.do_assemble()', () => {
    ...
    expect(doAssembleStmt.isMarker).toBe(true)
  ...
  it('拒绝 let 用于非 assemble 场景', () => {
    const code = `
      let foo = cad.box({ size: 20 })
    `
    expect(() => parseScript(code)).toThrow(/let.*only.*cad.assemble/)
  })
  ...
  it('完整链式调用 roundtrip', () => {
    ...
    // 验证再生成包含链式调用语法
    expect(regenerated).toContain('let assem1 = cad.assemble(')
  ...
  it('statementToLine 正确生成链式调用', () => {
    ...
    expect(assembleLine).toContain('let assem1 = cad.assemble(')
