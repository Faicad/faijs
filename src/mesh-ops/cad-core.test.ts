 ...
 * cad-core API 单元测试
 *
 * 验证 cad-core 的每个 API 正确包装了底层纯函数。
 * P1 阶段：cad-core 是薄包装层，行为与底层函数完全一致。
 *
...
describe('cad-core: immutability', () => {
  it('translate 不修改输入', () => {
    const s = cad.box({ size: 10 })
...
describe('cad-core: API contract', () => {
...
describe.skip('cad-core: boolean (requires Worker)', () => {
...
describe.skip('cad-core: split (requires Worker)', () => {
...
describe.skip('cad-core: drill (requires Worker)', () => {
...
describe.skip('cad-core: extrude (requires Worker)', () => {
