 ...
 *
 * 运行：npx vitest run src/renderer/engine/cad-core/primitives-brep.test.ts
 */

// ─── OCCT stdout 噪声过滤（与 primitiveToCad.test.ts 一致） ───
...
describe('BREP primitives: STEP export has precise surfaces', () => {
  it('boxBrep: underlying solid exports as ADVANCED_FACE with PLANE', async () => {
    // BREP 函数内部用 primitiveToCadSolid 构造实体，我们直接验证实体导出
    const result = primitiveToCadSolid(kernel, 'cube', { size: 20 })
    try {
      const step = cadSolidToStep(kernel, result.solid)
  ...
  it('cylinderBrep: underlying solid has CYLINDRICAL_SURFACE', async () => {
    const result = primitiveToCadSolid(kernel, 'cylinder', { radius: 10, height: 20 })
    try {
      const step = cadSolidToStep(kernel, result.solid)
  ...
  it('coneBrep: underlying solid has CONICAL_SURFACE', async () => {
    const result = primitiveToCadSolid(kernel, 'cone', { radiusBottom: 10, radiusTop: 0, height: 20 })
    try {
      const step = cadSolidToStep(kernel, result.solid)
  ...
  it('sphereBrep: underlying solid has SPHERICAL_SURFACE', async () => {
    const result = primitiveToCadSolid(kernel, 'sphere', { radius: 10 })
    try {
      const step = cadSolidToStep(kernel, result.solid)
...
describe('primitiveToCadSolid: backward compat with size number', () => {
  it('cube with size=20 (number) works', () => {
    const result = primitiveToCadSolid(kernel, 'cube', 20)
    try {
  ...
  it('cube with params object { size: 30 } works', () => {
    const result = primitiveToCadSolid(kernel, 'cube', { size: 30 })
    try {
      const bb = kernel.getBoundingBox(result.solid, false)
  ...
  it('cylinder with params object { radius: 5, height: 15 } works', () => {
    const result = primitiveToCadSolid(kernel, 'cylinder', { radius: 5, height: 15 })
    try {
      const bb = kernel.getBoundingBox(result.solid, false)
  ...
  it('cone with params object { radiusBottom: 8, radiusTop: 3, height: 12 } works', () => {
    const result = primitiveToCadSolid(kernel, 'cone', { radiusBottom: 8, radiusTop: 3, height: 12 })
    try {
      const bb = kernel.getBoundingBox(result.solid, false)
  ...
  it('cube with center offset via params', () => {
    const result = primitiveToCadSolid(kernel, 'cube', { size: 10, center: [50, 0, 0] })
    try {
      const bb = kernel.getBoundingBox(result.solid, false)
  ...
  it('sphere with params object { radius: 7 } works', () => {
    const result = primitiveToCadSolid(kernel, 'sphere', { radius: 7 })
    try {
      const bb = kernel.getBoundingBox(result.solid, false)
