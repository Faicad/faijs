 ...
 */
export interface PrimitiveToCadResult {
  /** CAD 实体句柄（调用方负责释放） */
  solid: ShapeHandle
  /** 使用的构造路径 */
  path: 'primitive' | 'mesh'
 ...
 * @returns CAD 实体 + 路径信息
 */
export function primitiveToCadSolid(
  kernel: OcctKernel,
  type: PrimitiveType | 'screw' | 'text',
  params?: PrimitiveParams | number,
): PrimitiveToCadResult {
  ...
  switch (type) {
    ...
    case 'text': {
      ...
      // 对于 screw/text，调用方应使用 geometryToCadSolid
      throw new Error(
        `${type} requires geometry data — use geometryToCadSolid() instead`,
      )
    }
 ...
 * @returns CAD 实体 + 路径信息
 */
export function geometryToCadSolid(
  kernel: OcctKernel,
 ...
 */
export function cadSolidToStep(kernel: OcctKernel, solid: ShapeHandle): string {
 ...
 *
 * 内部调用 primitiveToCadSolid + cadSolidToStep，自动管理句柄生命周期。
 *
 * @param kernel  已初始化的 OCCT 内核
 * @param type    primitive 类型
 ...
 */
export function primitiveToStep(
  kernel: OcctKernel,
  type: PrimitiveType,
  params?: PrimitiveParams | number,
): { step: string; path: 'primitive' | 'mesh' } {
  const result = primitiveToCadSolid(kernel, type, params)
  try {
