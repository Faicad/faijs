...
import { transformFaqts } from '../../../src/faqts/transform'
import { findImports, rewriteImports } from '../../../src/faqts/imports'
import { runFaqts } from '../../../src/faqts/run'
import { executeFaqtsModuleInNode } from '../../../src/faqts/exec-node'
import { cad as hostCad } from '../../../src/mesh/index'
import type { Shape } from '../../../src/mesh/types'

const FIXTURE = resolve(process.cwd(), 'test/faijs/faqts/fixtures/mount-plate.ts')
// faits 脚本经 rewrite 指向 mesh 入口（与宿主 cad 同源；纯 mesh 无需 OCCT）
const SRC_MESH_URL = pathToFileURL(resolve(process.cwd(), 'src/mesh/index.ts')).href
...
describe('faqts imports 解析处', () => {
  ...
  it('相对说明符按 baseURL 绝对化', () => {
    ...
    const base = pathToFileURL(resolve(process.cwd()) + '/').href
